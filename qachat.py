...
# Functionality for Feelings Questionnaire Page
    ...
    if submit_responses_button:
        selected_feelings = [feeling for feeling, selected in responses.items() if selected]
        if selected_feelings:
            combined_feelings = " ".join(selected_feelings)
            with st.spinner("MindMateAI Analyzing your feelings... 🤖"):
                response = get_gemini_response(combined_feelings)
            st.markdown(f"#### 🤖 MindMateAI Response: {response}")
        else:
            st.warning("Please select at least one feeling.")
